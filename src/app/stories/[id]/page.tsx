  ...
  if (!story) {
    return (
      <div className="page space-y-4">
        <p className="text-sm text-ink-soft">Story not found.</p>
        <Link href="/stories" className="text-sm font-semibold text-accent">
          ← All stories
        </Link>
      </div>
    );
    ...
    <div className="page space-y-6">
      <div>
        ...
        <p className="mt-3 text-[0.65rem] font-semibold uppercase tracking-wide text-accent">
          {story.theme} · {story.testament === "OT" ? "Old Testament" : "New Testament"}
        </p>
      ...
      {preview.length > 0 && (
        <section className="rounded-2xl border border-line bg-paper-elevated px-4 py-4">
          ...
          <div className="mt-3 space-y-2">
            {preview.map((text, i) => {
              const verseNum =
                (story.passages[0].verseStart ?? 1) + i;
              return (
